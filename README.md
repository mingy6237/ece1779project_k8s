...
Our project aims to solve these problems by building a **cloud-based Inventory Management System** that keeps all data **up to date and consistent**. By using **containers**, **Kubernetes**, and **edge deployment**, the system allows updates to appear instantly everywhere. It is also more reliable because it can automatically fix minor issues, back up data, and scale up on demand.
...
- **Deliver a responsive web experience** with instant stock updates, secure authentication, and real-time low-stock/adjustment notifications.
...
- **Backend**
  ...
  - Caching: **Redis** for inventory query caching and performance.
...
- **SKU and Inventory Management**
  - Full SKU lifecycle via `/api/manager/skus*`: list with pagination and filters, categories, detail view, create, update, delete.
  ...
  - The frontend exposes:
    ...
    - Item detail page with overview, per‑location quantities, stock history, and an audit log tab.

- **Real‑Time Inventory Updates and Alerts**
  - Outbox pattern and Kafka topics propagate inventory changes between backend instances.
...
- **Dashboard, Audit, and Reporting**
  ...
  - Low‑stock widget (manager‑only) links directly to affected items.
  - Audit log page surfaces chronologically ordered stock adjustments, with filters for item, location, and date range.
...
- **Advanced Feature – Real‑Time Functionality**
  - WebSocket hub and Kafka‑backed outbox pattern deliver **live inventory updates and alerts** to all connected sessions.
  - The frontend highlights updated rows, updates counters, and shows new alerts without requiring manual refresh.
...
- **Dashboard** – Overview metrics and low‑stock widget.
- **Items** – Inventory items list with filters and search.
- **Audit Log** – Full history of stock adjustments.
- **Alerts** – Low‑stock and out‑of‑stock alerts.
- **Profile / User Menu** – Current user info and logout.
...
- **Browsing and Managing Items**
  ...
  - Click on a row to open the **Item Detail** page, where you can view overview, locations, stock history, and audit log; managers can edit or delete.
